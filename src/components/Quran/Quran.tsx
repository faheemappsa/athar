...
const SURAHS = [
  ...
  ["النازعات", 583], ["عبس", 585], ["التكوير", 586], ["الانفطار", 587], ["المطففين", 587], ["الانشقاق", 589],
  ...
  return (
    ...
    >
      ...
      <div className="mt-3 rounded-[28px] bg-white p-2 shadow-sm ring-1 ring-black/5">
        <select
          aria-label="اختر سورة"
          value=""
          onChange={(event) => goToPage(Number(event.target.value))}
          className="mb-2 h-12 w-full rounded-full border border-secondary-text/20 bg-primary-bg/40 px-4 text-center text-sm font-bold text-primary-text focus:border-action focus:bg-white focus:outline-none"
        >
          <option value="" disabled>اختر السورة</option>
          {SURAHS.map(([name, startPage]) => (
            <option key={name} value={startPage}>{name}</option>
          ))}
        </select>
